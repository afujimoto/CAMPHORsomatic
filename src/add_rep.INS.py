...
#		print(line)
			if int(line[2]) > int(row_l[1]) - range_len >= int(line[1]) and int(line[1]) < int(row_l[3]) + range_len <= int(line[2]):
				coverage = "complete"
	#		coverage_prop = str(round((abs(int(line[2]) - int(line[1])))/(abs(int(row_l[3]) - int(row_l[1]))), 2))
				coverage_prop = str(1.00)
				out_tmp = [",".join(line), str(coverage_prop), coverage]
				out_tmp_str = ",".join(out_tmp)
				out.append(out_tmp_str)

			elif int(line[1]) < int(row_l[1]) - range_len < int(line[2]) and int(row_l[3]) + range_len > int(line[2]):
				coverage = "partial"
				coverage_prop = 0
				if abs(int(row_l[3]) - int(row_l[1])):
			...
			elif int(row_l[1]) - range_len < int(line[1]) and int(line[1]) < int(row_l[3]) + range_len < int(line[2]):
				coverage = "partial"
				coverage_prop = 0
				if abs(int(row_l[3]) - int(row_l[1])):
			...
			elif int(row_l[3]) + range_len > int(line[1]) > int(row_l[1]) - range_len and int(row_l[1]) - range_len < int(line[2]) < int(row_l[3]) + range_len:
				coverage = "within"
				coverage_prop = 0
				if abs(int(row_l[3]) - int(row_l[1])):
					coverage_prop = str(round((abs(int(line[2]) - int(line[1])))/(abs(int(row_l[3]) - int(row_l[1]))), 2))
